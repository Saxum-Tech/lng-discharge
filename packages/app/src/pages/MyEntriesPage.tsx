...
import type { Flight, CruiseSchedule } from '@lng/shared';
...
export function MyEntriesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: f }, { data: c }] = await Promise.all([
      supabase.from('flights').select('*').eq('company_id', companyId).order('scheduled_arrival', { ascending: false }),
      supabase.from('cruise_schedules').select('*').eq('company_id', companyId).order('arrival_date', { ascending: false }),
    ]);
    setFlights(f ?? []);
    setCruises(c ?? []);
    setLoading(false);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle><span className="flex items-center gap-2"><Plane size={16} /> Flights ({flights.length})</span></CardTitle>
            </CardHeader>
            {flights.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {flights.map((f) => (
                  <div key={f.id} className="flex items-center justify-between py-3">
                    <div>
                      <p className="font-medium text-gray-900">{f.flight_number} — {f.origin} → {f.destination}</p>
                      <p className="text-xs text-gray-400">
                        Arrival: {format(parseISO(f.scheduled_arrival), 'dd MMM yyyy HH:mm')}
                        {f.is_private && <span className="ml-2 rounded bg-gray-100 px-1 py-0.5 text-gray-500">Private</span>}
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(f, 'flight')} className="rounded p-1 text-gray-400 hover:text-[var(--color-primary)]"><Pencil size={14} /></button>
                      <button onClick={() => deleteFlight(f.id)} className="rounded p-1 text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle><span className="flex items-center gap-2"><Ship size={16} /> Vessels ({cruises.length})</span></CardTitle>
            </CardHeader>
            {cruises.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {cruises.map((c) => (
                  <div key={c.id} className="flex items-center justify-between py-3">
                    <div>
                      <p className="font-medium text-gray-900">{c.vessel_name}{c.vessel_type ? ` (${c.vessel_type})` : ''}</p>
                      <p className="text-xs text-gray-400">
                        Arrival: {format(parseISO(c.arrival_date), 'dd MMM yyyy HH:mm')}
                        {c.is_private && <span className="ml-2 rounded bg-gray-100 px-1 py-0.5 text-gray-500">Private</span>}
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(c, 'cruise')} className="rounded p-1 text-gray-400 hover:text-[var(--color-primary)]"><Pencil size={14} /></button>
                      <button onClick={() => deleteCruise(c.id)} className="rounded p-1 text-gray-400 hover:text-red-600"><Trash2 size={14} /></button>
                    </div>
                  </div>
                ))}
              </div>
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (type === 'flight') {
        ...
        if (editItem) {
          const { error } = await supabase.from('cruise_schedules').update(payload).eq('id', editItem.id);
          if (error) throw error;
        } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded p-1 text-gray-400 hover:text-gray-600"><X size={18} /></button>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is-private" className="text-sm text-gray-700">Private (only visible to your company)</label>
          </div>
