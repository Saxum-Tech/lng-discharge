...
export function AnalysisPage() {
  ...
  function handleExport() {
    ...
    downloadCSV(`discharge-windows-${year}-${String(month).padStart(2, '0')}.csv`, toCSV(rows));
  }
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="secondary" size="sm" onClick={handleExport} disabled={windows.length === 0}>
      ...
      {longest && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-6 py-4">
          ...
          <p className="mt-1 text-3xl font-bold text-amber-800">{formatDuration(longest.duration_hours)}</p>
      ...
      <Card>
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-400">
            No discharge windows found for this month.
          </p>
        ) : (
                ...
                {windows.map((w, i) => (
                  ...
                  >
                    <td className="py-3 pr-4 font-medium">
                      <Link to={`/day/${w.date}`} className="text-[var(--color-primary)] hover:underline">
                        {format(new Date(w.date), 'EEE d MMM')}
                      </Link>
                    </td>
                    <td className="py-3 pr-4 text-gray-600">{format(new Date(w.start_time), 'HH:mm')}</td>
                    <td className="py-3 pr-4 text-gray-600">{format(new Date(w.end_time), 'HH:mm')}</td>
                    <td className="py-3 pr-4 font-semibold text-gray-900">{formatDuration(w.duration_hours)}</td>
                    <td className="py-3">
