...
export function ProfilePage() {
  ...
  async function handlePasswordChange(e: React.FormEvent) {
    ...
    setSaving(false);
    if (error) setError(error.message);
    else { setMessage('Password updated.'); setCurrentPassword(''); setNewPassword(''); }
  }
        ...
        <Card>
          ...
          <dl className="divide-y divide-gray-100 text-sm">
            ...
            <div className="flex justify-between py-2">
              ...
              <dd className="font-medium text-gray-900">{roleLabel[profile?.role ?? 'viewer']}</dd>
            </div>
            <div className="flex justify-between py-2">
              <dt className="text-gray-500">Company</dt>
              <dd className="font-medium text-gray-900">{(profile?.company as { name?: string } | null)?.name ?? '—'}</dd>
            </div>
          </dl>
        </Card>
