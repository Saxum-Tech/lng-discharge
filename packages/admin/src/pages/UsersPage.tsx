...
import type { Profile, Company, UserRole } from '@lng/shared';
...
export function UsersPage() {
  ...
  async function fetchData() {
    ...
    const [{ data: profiles }, { data: comps }] = await Promise.all([
      supabase.from('profiles').select('*, company:companies(id, name)').order('created_at', { ascending: false }),
  ...
  async function sendMagicLink(u: UserWithEmail) {
                    ...
                    <td className="py-3">
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className={`rounded p-1 ${u.is_active ? 'text-gray-400 hover:text-red-600' : 'text-gray-400 hover:text-green-600'}`}
...
function InviteModal({
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    // Using admin API via the edge function or server action in real deployment
